...
namespace GameBaseArilox.UnitTest
{
    public class TestParticleGenerator : GameModel
    {
        ...
        {
            Dictionary<string, int> spritePercentage = new Dictionary<string, int>
            {
                {"dustParticle", 100},
                {"SpriteTest", 0}
            };
            _particleGenerator = new ParticleGenerator(400, 300, spritePercentage, Direction.Right, 0, 999, 0.2f, 10);
            GeneratorUpdater.AddGenerator(_particleGenerator);
            GeneratorUpdater.AddGenerator(new ParticleGenerator(250, 250, spritePercentage, Direction.Top, new Angle(45), 10, 0.5f, 4));
        }
